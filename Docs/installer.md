...
> **Кратко:** Графический установщик, который извлекает `NtoLib.dll` и сопутствующие файлы из zip-архива, разблокирует скачанные файлы, копирует их в каталог MasterSCADA, разворачивает конфигурацию по умолчанию и выполняет COM-регистрацию через `netreg.exe`. Требует прав администратора.
...
|---------|--------------|----------|
| Back up existing files before overwriting | Включён | При установке существующие файлы сохраняются в `C:\DISTR\Backup\NtoLib\<метка времени>\` перед перезаписью |
...
Рабочим каталогом служит DLL destination. Весь вывод `netreg.exe` (и штатные сообщения, и сообщения об ошибках) отображается в журнале. Если `netreg.exe` завершается с ненулевым кодом возврата, установка считается неудачной.
