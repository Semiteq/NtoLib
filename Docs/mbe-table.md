...
- [4. Контракт пинов](#4-контракт-пинов)
    - [4.1. Входные пины](#41-входные-пины)
    - [4.2. Выходные пины](#42-выходные-пины)
    - [4.3. Динамические пины оборудования](#43-динамические-пины-оборудования)
...
- **Динамические пины оборудования.** Пины-имена объектов (заслонки, нагреватели и т.п.)
...
- Загрузка конфигурации стартует либо по клику на объекте в дереве (требуется для отображения
  пинов), либо при переходе на экран с объектом — смотря что произойдёт раньше.
...
| ------------------- | -------- | ---------------------------------------------------------------------------------------- |
| Копировать строки   | `Ctrl+C` | Данные выделенных строк помещаются в буфер в формате TSV; копируются только колонки с `save_to_csv: true`. Доступно всегда |
...
- **Исполняемая строка** (`ActualLineNumber`, 0-based) подсвечивается; пройденные строки
...
Все входные пины должны быть в состоянии «Good» (содержать значение или иметь привязку к
...
| ActualLineNumber | int   | Номер текущей исполняемой строки (0-based). Подсвечивает текущую и пройденные строки |
...
### 4.3. Динамические пины оборудования

Помимо фиксированных пинов выше, блок создаёт **динамические строковые пины** имён оборудования
...
**Кодировки.** Чтение и запись — UTF-8 with BOM; недопустимые байты при чтении приводят к ошибке.
...
- значения парсятся в `InvariantCulture` (short — целое, float — с плавающей точкой; иной тип —
  ошибка);
- проверяется доступность целевых объектов (таргетов): для колонок с `group_name` значение должно
  ссылаться на существующий доступный объект.

После сборки выполняется проверка доступности таргетов; при ошибках рецепт возвращается вместе с
...
| `PinGroupDefs.yaml` | Группы пинов оборудования                             | 7.4    |
| `ActionsDefs.yaml`  | Типы действий (шагов)                                 | 7.5    |

**Путь к конфигурации:**

- По умолчанию: `C:\DISTR\Config\NtoLibTableConfig`.
- Обратная совместимость: если путь не задан явно, может использоваться
  `C:\Program Files (x86)\MPSSoft\MasterSCADA\NtoLibTableConfig`.
...
2. После загрузки всех файлов выполняется единый сквозной проход перекрёстных проверок: ссылки
   колонок на типы свойств; ссылки действий на колонки, типы и группы пинов; соответствие
...
хостовое приложение показывает MessageBox с деталями. Прекомпиляция формул выполняется отдельно
...
Описывает наборы пинов оборудования. Имена групп используются в `ActionsDefs.yaml` для колонок с
`property_type_id: "Enum"`.
...
| `first_pin_id` | да    | Первый пин группы, `> 0`; должен быть `>= pin_group_id`      |
| `pin_quantity` | да    | Количество пинов, `> 0`                                      |

Диапазоны пинов разных групп не должны пересекаться. Диапазон вычисляется как
...
- для `Enum` и `Time` проверка значения по умолчанию не выполняется; для `Enum` значением по
  умолчанию берётся целевой объект (таргет) с минимальным `id` среди непустых пинов группы,
...
1. **Проверка вместимости.** Требуемые объёмы (`rows * IntColumnCount` и
   ...
   выполняется.
2. **Подключение и валидация.** Устанавливается соединение, выполняется «пинг»: чтение R0 и сверка
...
1. **Подключение и валидация** — соединение и «пинг» R0 / Magic Number.
