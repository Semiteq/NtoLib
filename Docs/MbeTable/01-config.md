...
- Неизвестные/лишние поля в YAML игнорируются.
- Все проверки выполняются регистронезависимо.

## Состав конфигурации и порядок загрузки

Модуль ожидает четыре файла конфигурации (фиксированные имена) в каталоге, указанном в свойстве блока "Путь к конфигурационному каталогу":
...
**Путь к конфигурации:**
- По умолчанию для новых проектов: `C:\DISTR\Config\NtoLibTableConfig`
- Обратная совместимость: если путь не задан явно, используется `C:\Program Files (x86)\MPSSoft\MasterSCADA\NtoLibTableConfig`
- Можно изменить через визуальное свойство блока (изменения применяются при следующей загрузке блока)
...
4. Загружается и валидируется `ActionsDefs.yaml`, затем:
    - проверяются ссылки на колонки/типы/группы пинов из ранее загруженных файлов;
    - проверяется соответствие `default_value` ограничениям типов;
    - выполняется структурная проверка секции `formula` (наличие `expression`, валидность `recalc_order`, отсутствие
      дубликатов).
5. После успешной загрузки всех файлов выполняется валидация формул для действий, содержащих секцию `formula`:
    - проверяется, что все переменные из `recalc_order` существуют в списке колонок действия.

Любая ошибка на любом этапе останавливает процесс загрузки.

На успешной загрузке формируется объект конфигурации, который используется далее в приложении на протяжении всего
времени жизни.

Побочные эффекты при использовании фасада:

- Фасад конфигурации выбрасывает `ConfigException` с агрегированным списком ошибок; хостовое приложение отображает
  MessageBox и пробрасывает исключение.
- Прекомпиляция формул выполняется отдельно после загрузки конфигурации через `IFormulaPrecompiler`.
...
Назначение: описывает типы данных и их ограничения/форматирование. Идентификаторы из этого файла используются в
`ColumnDefs.yaml` и `ActionsDefs.yaml`.

Поддерживаемые системные типы:

- Строка: `System.String`
- Целое 16‑бит: `System.Int16` (
  см. [документацию](https://learn.microsoft.com/en-us/dotnet/api/system.int16?view=netframework-4.8.1))
- Число с плавающей точкой: `System.Single` (
  см. [документацию](https://learn.microsoft.com/en-us/dotnet/api/system.single?view=netframework-4.8.1))
...
- `business_logic.save_to_csv` — обязательное булево поле. Определяет, сохранять и загружать ли колонку из CSV‑файла
  рецепта.
- `ui.code` - обязателен
- `ui.ui_name` - обязателен
...
Назначение: описывает наборы пинов оборудования. Имена групп используются в `ActionsDefs.yaml` для колонок с
`property_type_id: "Enum"`.
...
Назначение: определяет доступные действия (команды), их длительность и набор задействованных колонок с типами и, при
необходимости, значениями по умолчанию.

ID действий, зарезервированные системой: 10 - `Wait`, 20 - `For`, 30 - `EndFor`, 40 - `Pause`. Из конфигурации можно
удалить эти действия, но нельзя создавать свои действия отличного назначения с тем же ID.
...
Формулы (опциональная секция `formula` в действии):

Структура (валидация на этапе загрузки `ActionsDefs.yaml`):

- `expression` — обязательна, непустая строка.
- `recalc_order` — обязательный список, не пустой, без дубликатов (регистронезависимо).

- Валидация после успешной загрузки всех файлов:
    - Все имена из `recalc_order` должны соответствовать ключам колонок в этом действии.

- Прекомпиляция выполняется вне процесса загрузки конфигурации (не в `ConfigurationLoader`):
    - Выражение должно корректно парситься (AngouriMath).
    - Все переменные из выражения должны быть в `recalc_order` (и наоборот).
    - Формула должна быть линейной по каждой переменной (каждая переменная встречается не более одного раза).
    - Должно быть возможно решить выражение относительно любой переменной из `recalc_order`.
...
- Если тип — числовой (`System.Int16` или `System.Single`), значение должно парситься и попадать в диапазон `min`/
  `max` (если заданы).
...
- Перед загрузкой проверяется наличие директории конфигурации (из свойства блока "Путь к конфигурационному каталогу" или используется значение по умолчанию) и каждого файла.
- При ошибках (отсутствие директории/файлов, ошибки валидации и т. п.) фасад выбрасывает `ConfigException` с
  агрегированными ошибками.
- Хостовое приложение отображает MessageBox с деталями из `ConfigException` и пробрасывает исключение.

Поведение парсера YAML:

- Пустой файл — ошибка.
- Ошибка парсинга YAML — ошибка.
- Лишние (неизвестные) поля в YAML — игнорируются.

## Ошибки и коды ошибок

## Ошибки и формат сообщений

- Коды ошибок не используются.
- Каждая ошибка содержит:
    - `section` — имя секции/файла (например, `ColumnDefs.yaml`);
    - `context` — конкретный контекст (например, ключ колонки/действия);
    - Дополнительные метаданные (например, `field`, `value`, `min`, `max`).
- Примеры сообщений:
...
Ошибки прекомпиляции формул:
- "Failed to compile formula: Formula expression is empty."
- "Failed to compile formula: Recalculation order is missing variables: ..."
