...
- Для каждой другой колонки:
    - Если колонка не применима к выбранному действию, но в ячейке есть значение — предупреждение (значение
      игнорируется, импорт продолжается). Каждое такое несоответствие логируется индивидуально через Serilog, а по
      итогам сборки формируется одно суммарное предупреждение со списком затронутых строк, которое отображается в
      строке статуса.
    - Если колонка не применима к выбранному действию и ячейка пуста — значение пропускается без предупреждения.
    - Пустые значения применимых колонок заполняются значениями по умолчанию из конфигурации YAML.
...
- При первой ошибке сборка прерывается (исключение — неприменимые колонки с непустыми значениями, которые
  обрабатываются как предупреждения и не прерывают сборку).
