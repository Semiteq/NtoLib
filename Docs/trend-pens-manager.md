...
> **Кратко:** По команде Execute очищает все перья в открытом окне тренда и заново добавляет их на основе структуры сервисов под `DataRootPath`. Для сервисов БКТ, БП, БУЗ имена перьев берутся из ConfigLoader. Для остальных -- имя пера = имя параметра. Обрабатываются только каналы с `Used == true`. Окно тренда должно быть открыто.
...
- Каждый канал обязан содержать вход `Used` (bool). Отсутствие `Used` -- критическая ошибка.
- Остальные входы канала -- параметры для перьев.
...
| Отсутствует вход `Used` в канале | `Errors` содержит описание, перья не добавляются |
| Группа ConfigLoader не найдена или вход отсутствует | `Errors` содержит описание, перья не добавляются |
...
| Ситуация | Поведение |
|----------|-----------|
| Сигнал не найден, нет прав, сбой добавления | Ошибка накапливается, остальные перья продолжают обрабатываться |
