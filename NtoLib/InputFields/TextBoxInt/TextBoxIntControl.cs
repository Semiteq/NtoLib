...
using NtoLib.InputFields.Helpers;
...
public partial class TextBoxIntControl : VisualControlBase
{
	...
	protected override void OnPaint(PaintEventArgs e)
	{
		...
		if (!DesignMode)
		{
			...
			var input = FBConnector.GetPinInt(TextBoxIntFB.OutputToControlId);
			if (input != _lastInput)
			{
				_lastInput = input;
				_value = input;
			...
			if (_isInitialized)
			{
				...
				FBConnector.SetPinValue(TextBoxIntFB.ValidationAboveMaxId, _validationAboveMax);
				FBConnector.SetPinValue(TextBoxIntFB.ValidationBelowMinId, _validationBelowMin);
				FBConnector.SetPinValue(TextBoxIntFB.ValidationParseErrorId, _validationParseError);
			}
		}
	}
	...
	{
		var actualIsLocked = _inputIsLock || UserLock;
	...
	private void ToEditMode()
	{
		...
		var beforeLenght = TextBefore.Length;
		if (beforeLenght > 0)
			beforeLenght++;

		var afterLenght = TextAfter.Length;
		if (afterLenght > 0)
			afterLenght++;
	...
	{
		var readResult = ReadValue(textBox.Text, out var value);
