...
Функциональный блок автоматически формирует последовательность перьев и добавляет их в окно трендов на основании структуры сервисов в дереве и данных из ConfigLoader.
...
Если для указанного сервиса нет данных ConfigLoader или индекс канала выходит за пределы массива — это считается **критической ошибкой**, последовательность не строится.
...
- `Warnings` — немасштабные предупреждения (например, нестандартные конструкции, не влияющие на корректность).
...
### Шаг 4: Построение последовательности перьев
...
- Сервисы, для которых соответствующий флаг `Add... == false`, не участвуют в последовательности вообще.
- Для сервисов типов БКТ, БП, БУЗ:
...
- При первом обнаруженном критическом несоответствии (конфликт размерностей, отсутствующие данные) построение последовательности прерывается, перья не добавляются.

### Шаг 5: Применение последовательности к тренду
...
- **Повторный запуск:**  
  При каждом запуске:
    ...
    - Последовательность строится заново по текущему состоянию дерева `DataRootPath`, параметрам `Add...` и состоянию ConfigLoader.
