...
1.  **Установка библиотеки:**
    *   Скопируйте библиотеку `NtoLib.dll` и все сопутствующие зависимости в корневую папку установки MasterSCADA.
    *   Зарегистрируйте сборку с помощью утилиты `netreg.exe`.

2.  **Создание папки конфигурации:**
    *   В корневой директории вашего проекта MasterSCADA создайте папку с именем `NtoLibTableConfig`.
...
*   **Пример:**
    ...
    - property_type_id: "Temp"      # Уникальный ID. Используется в ColumnDefs и ActionsDefs.
      ...
      format_kind: "Auto"           # Формат отображения. Варианты: Auto, Scientific, TimeHms. Auto выводит данные в формате 123.## с опциональной дробной частью, Scientific 123.##E123, TimeHms 01:02:03.###. Значения, введенные с большим количеством знаков после запятой, будут округлены до 2 знаков при выводе в UI, но запишутся в файл.
...
*   **`debug_logger.txt`**: Файл создается автоматически в той же директории, где расположена библиотека `NtoLib.Recipes.MbeTable.dll`. В него записываются все ключевые операции: загрузка конфигурации, ошибки валидации, процесс обмена с ПЛК и т.д.