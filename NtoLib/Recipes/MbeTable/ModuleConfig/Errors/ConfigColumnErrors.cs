...
public static class ConfigColumnErrors
{
	...
	{
		return new ConfigError($"Missing mandatory columns: {missingList}.", Section, "mandatory-columns")
			.WithDetail("MissingColumns", missingList);
	}

	public static ConfigError UnsupportedTypes(string info)
	{
		return new ConfigError($"Unsupported column types: {info}.", Section, "column-type-check")
	...
	{
		return new ConfigError($"max_dropdown_items must be > 0. Actual: {value}.", Section, context)
	...
	{
		return new ConfigError("column_type 'action_combo_box' can only be used with key='action'.", Section, context)
			.WithDetail("columnKey", columnKey)
