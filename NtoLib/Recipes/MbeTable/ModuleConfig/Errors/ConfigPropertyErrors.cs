...
public static class ConfigPropertyErrors
{
	...
	{
		return new ConfigError($"Unknown SystemType '{systemType}'.", Section, context)
	...
	public static ConfigError UnsupportedSystemType(string propertyTypeId, string systemType, string supported,
		string context)
	{
		return new ConfigError($"Unsupported SystemType '{systemType}'. Supported: {supported}.", Section, context)
			.WithDetail("PropertyTypeId", propertyTypeId)
			.WithDetail("SystemType", systemType)
	...
	public static ConfigError UnsupportedFormatKind(string propertyTypeId, string formatKind, string supported,
		string context)
	{
		return new ConfigError($"Unsupported format_kind '{formatKind}'. Supported: {supported}.", Section, context)
			.WithDetail("PropertyTypeId", propertyTypeId)
	...
	{
		return new ConfigError("format_kind='TimeHms' can only be used with PropertyTypeId='Time'.", Section, context)
			.WithDetail("PropertyTypeId", propertyTypeId)
	...
	{
		return new ConfigError("format_kind='Scientific' can only be used with numeric types.", Section, context)
			.WithDetail("PropertyTypeId", propertyTypeId)
			.WithDetail("SystemType", systemType)
