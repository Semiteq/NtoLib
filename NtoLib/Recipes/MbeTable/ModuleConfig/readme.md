...
4. Загружается и валидируется `ActionsDefs.yaml`, затем:
   ...
   - выполняется структурная проверка секции `formula` (наличие `expression`, валидность `recalc_order`, отсутствие дубликатов и наличие перечисленных колонок).
5. Выполняется прекомпиляция формул из `ActionsDefs.yaml`. Любые ошибки компиляции конвертируются в `ConfigError` с `section="ActionsDefs.yaml"` и `context="ActionId=..., ActionName='...'"`. Результат компиляции кэшируется и переиспользуется.
...
- Ошибки компиляции формул обрабатываются в том же пайплайне и в том же формате, что и остальные ошибки конфигурации.
...
- Структура:
  - `expression` — обязательна, непустая строка математического выражения.
  - `recalc_order` — обязательный список из как минимум 2 уникальных имен переменных (регистронезависимо).
    +- Ссылочная целостность:
  - Все имена в `recalc_order` должны соответствовать ключам колонок в этом действии.
  - Набор переменных в `expression` должен совпадать с набором из `recalc_order` (без лишних/пропущенных).
    +- Компиляция выполняется при загрузке конфигурации:
  - Выражение должно корректно парситься.
  - Формула должна быть линейной по каждой переменной и разрешимой для пересчета любой переменной из `recalc_order`.
  - При ошибке компиляции загрузка конфигурации завершается с `ConfigException` (см. раздел «Ошибки...»).
...
- Примеры сообщений:
    ...
    - "ActionsDefs.yaml, ActionId=..., ActionName='...': Failed to compile formula: ... "
