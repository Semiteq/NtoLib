...
Модуль ожидает четыре файла конфигурации (фиксированные имена):
...
4. Загружается и валидируется `ActionsDefs.yaml`, затем:
   ...
   - выполняется структурная проверка секции `formula` (наличие `expression`, валидность `recalc_order`, отсутствие дубликатов).
...
- `business_logic.save_to_csv` — обязательное булево поле. Определяет, сохранять и загружать ли колонку из CSV‑файла рецепта.
- `ui.code` - обязателен
...
- Перед загрузкой проверяется наличие директории и каждого файла.
