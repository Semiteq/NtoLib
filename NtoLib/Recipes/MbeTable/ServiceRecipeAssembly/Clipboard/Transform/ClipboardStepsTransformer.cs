    ...
    public Result<IReadOnlyList<Step>> Transform(IReadOnlyList<PortableStepDto> dtos)
    {
        ...
        for (int i = 0; i < dtos.Count; i++)
        {
            ...
            if (actionResult.IsFailed)
                return Result.Fail<IReadOnlyList<Step>>(new ClipboardTransformFailedError(i, $"ActionId {dto.ActionId} not found"))
            ...
            {
                var columnDef = actionDef.Columns.FirstOrDefault(c => c.Key.Equals(kv.Key, StringComparison.OrdinalIgnoreCase));
                if (columnDef == null)
                {
                ...
                {
                    return Result.Fail<IReadOnlyList<Step>>(new ClipboardTransformFailedError(i, $"Failed to parse value '{kv.Value}' for column '{kv.Key}'"))
                ...
                {
                    return Result.Fail<IReadOnlyList<Step>>(new ClipboardTransformFailedError(i, $"Failed to set column '{kv.Key}'"))
