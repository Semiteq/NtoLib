...
public sealed class TargetAvailabilityValidator
{
		...
		List<string> errors)
	{
		...
		return ValidateTargetExists(stepIndex, actionId, actionName, column.Key, column.GroupName!, targetId, snapshot,
			errors);
	}
		...
		List<string> errors)
	{
		...
		{
			errors.Add(new ValidationTargetNotFoundError(stepIndex, actionId, actionName, columnKey, targetId,
				groupName).Message);
		}
