...
namespace NtoLib.Recipes.MbeTable.RecipeManager
{
    ...
    public sealed class RecipeEngine
    {
        ...
        public RecipeEngine(ActionManager actionManager, StepFactory stepFactory,
            IActionTargetProvider actionTargetProvider, IReadOnlyDictionary<int, Func<int, Step>> actionToFactoryMap,
        ...
        public Recipe ReplaceStepWithNewDefault(Recipe currentRecipe, int rowIndex, int newActionId)
        {
            ...
            if (!_actionToFactoryMap.TryGetValue(newActionId, out var creationFunc))
                throw new ArgumentException($"No step creation function found for action ID {newActionId}.");
            ...
            Step currentStep, ColumnKey columnKey, object value)
        {
            ...
            if (!success) return (currentStep, error);
