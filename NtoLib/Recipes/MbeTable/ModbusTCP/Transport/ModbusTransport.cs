...
namespace NtoLib.Recipes.MbeTable.ModbusTCP.Transport;
...
internal sealed class ModbusTransport : IModbusTransport
{
    ...
    public async Task<Result> EnsureConnectedAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            var currentConnectionString = $"{settings.IpAddress}:{settings.Port}";
    ...
    public Task<Result<int[]>> ReadHoldingAsync(int address, int length, CancellationToken ct) =>
        ExecuteAsync(() => _client!.ReadHoldingRegisters(address, length),
            length, "Read", ct);
    ...
    private async Task<Result> ConnectAndPingAsync(RuntimeOptions settings, CancellationToken ct)
    {
        ...
        {
            DisconnectInternal();
            return Fail(ErrorCode.PlcConnectionFailed, ex.Message);
        }
        catch (ModbusException mex)
        {
            DisconnectInternal();
            return Fail(ErrorCode.ModbusTcpLibraryError, mex.Message);
        }
    }
    ...
    private async Task<Result> PingDirectAsync(RuntimeOptions settings, CancellationToken ct)
    {
        ...
        {
            return Fail(ErrorCode.PlcInvalidResponse,
    ...
    private async Task<Result<T>> ExecuteOperationAsync<T>(Func<T> operation, CancellationToken ct)
    {
        ...
        {
            DisconnectInternal();
            return Fail(ErrorCode.PlcTimeout, ex.Message).ToResult<T>();
        }
        catch (ModbusException mex)
        {
            return Fail(ErrorCode.ModbusTcpLibraryError, mex.Message).ToResult<T>();
        }
    }
    ...
    private static Result Fail(ErrorCode code, string message) =>
        Result.Fail(new Error(message).WithMetadata(nameof(ErrorCode), code));
}