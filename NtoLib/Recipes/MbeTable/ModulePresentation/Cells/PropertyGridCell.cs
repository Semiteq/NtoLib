...
/// </summary>
public sealed class PropertyGridCell : DataGridViewTextBoxCell 
{
    /// <summary>
        ...
        TypeConverter? valueTypeConverter)
    {
        ...
        if (targetType != typeof(short) && targetType != typeof(float))
        {
        ...
        {
            if (TryParseShort(token, out var intValue))
                return intValue;
    ...
    private static bool TryParseShort(string token, out short value)
    {
        if (short.TryParse(token, NumberStyles.Integer, CultureInfo.CurrentCulture, out value))
            return true;
        if (short.TryParse(token, NumberStyles.Integer, CultureInfo.InvariantCulture, out value))
            return true;
        return false;
    }
