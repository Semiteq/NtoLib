...
public sealed class TableInputManager : IDisposable
{
    ...
    public void Detach()
    {
        ...
        try { _table.KeyDown -= OnTableKeyDown; } catch { /* ignored */ }
        try { _table.RowHeaderMouseClick -= OnRowHeaderMouseClick; } catch { /* ignored */ }

        if (_rowHeaderMenu != null)
        {
            try { _rowHeaderMenu.Dispose(); } catch { /* ignored */ }
            _rowHeaderMenu = null;
        }
