...
namespace NtoLib.Recipes.MbeTable.RecipeLines
{
    ...
    {
        private readonly Dictionary<string, Func<int, float, float, float, float, string, RecipeLine>> _creators = new()
        {
        ...
        public RecipeLine NewLine(string actionName, int actionTarget, float setpoint, float initialValue, float speed, float timeSetpoint, string comment)
        {
        ...
        {
            var actionName = ActionManager.GetActionNameById((int)intData[index * 2]).ToString();
            var actionTarget = intData[index * 2 + 1];

            var initialValue = 0; // todo: get from somewhere
            var speed = 0;
