...
public sealed class CoreForLoopUnmatchedWarning : BilingualWarning
{
                ...
                : $"Незакрытый ForLoop на шаге {stepIndex + 1}: {details}")
    {
        ...
        Metadata["stepIndex"] = stepIndex;
        if (startIndex.HasValue) Metadata["startIndex"] = startIndex.Value;
        if (endIndex.HasValue) Metadata["endIndex"] = endIndex.Value;
        if (!string.IsNullOrEmpty(details)) Metadata["details"] = details;
    }
}