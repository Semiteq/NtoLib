...
- Свойство блока "Записывать лог в файл"
    - Включено — логирование активно.
    - Выключено — логирование полностью отключено (нет вывода ни в файл, ни в отладочный вывод/консоль).
- Свойство блока "Путь к каталогу логов"
    - Каталог для лог‑файлов. Допустимо использовать переменные окружения (например, `%AppData%`, `%USERPROFILE%`).
    - Значение по умолчанию: `C:\DISTR\Logs`
    - Старые проекты закешировавшие `%AppData%/NtoLibLogs` будут продолжать использовать этот путь, если не изменить его вручную.
## Имя и расположение файла

- Имя файла: `mbe-table.log`
- Путь по умолчанию: `C:\DISTR\Logs\mbe-table.log`
