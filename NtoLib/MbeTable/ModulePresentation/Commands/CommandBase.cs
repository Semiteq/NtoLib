...
public abstract class CommandBase
{
	...
	protected async Task<Result> ExecuteWithBusyAsync(Func<CancellationToken, Task<Result>> runner,
		CancellationToken ct)
	{
		if (!CanExecute())
			return Result.Fail(new PresentationUiBusyError());
		...
		using (_busy.Enter(GetOperationKind()))
		{
			...
			{
				return Result.Fail(new PresentationOperationCanceledError());
			}
			catch (Exception ex)
			{
				return Result.Fail(new PresentationUnexpectedError(ex.Message));
			}
		}
	}
}
