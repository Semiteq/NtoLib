...
public sealed class ConnectRunnerTests
{
	...
	{
		var logger = new CapturingSink().ToLogger();
		...
		var (issuedCount, threw) = ConnectRunner.ConnectAll(commands, logger);

		issuedCount.Should().Be(3);
		threw.Should().Be(0);
	...
	public void ConnectAll_ThrowingCommand_IsCaughtCountedAndTheRestStillRun()
	{
		...
		errors[0].Exception.Should().BeOfType<InvalidOperationException>();
		errors[0].MessageTemplate.Text.Should()
			.Be("Connect {LocalPin} <-> {ExternalPin} ({LinkType}) threw");
		CapturingSink.Property(errors[0], "LocalPin").Should().Be("Root.B");
		CapturingSink.Property(errors[0], "ExternalPin").Should().Be("Ext.B");
		sink.Events.Should().NotContain(e => e.Level == LogEventLevel.Warning);

		sink.Events.Count(e => e.MessageTemplate.Text.StartsWith("Connect issued", StringComparison.Ordinal))
			.Should().Be(issuedCount, "the summary counter and the per-link records must agree");
	}
	...
	public void ConnectAll_NoThrows_ReportsZeroThrewAndNoWarning()
	{
		...
		issuedCount.Should().Be(2);
		threw.Should().Be(0);
		sink.Events.Should().NotContain(e => e.Level == LogEventLevel.Error);
	}

	[Fact]
	public void ConnectAll_EveryCommandThrows_IssuesNothingAndLogsOneErrorEach()
	{
		var sink = new CapturingSink();
		var logger = sink.ToLogger();
		...
		issuedCount.Should().Be(0);
		threw.Should().Be(2);
		sink.Events.Count(e => e.Level == LogEventLevel.Error).Should().Be(2);
		sink.Events.Should().NotContain(e =>
			e.MessageTemplate.Text.StartsWith("Connect issued", StringComparison.Ordinal));
	}
	...
	public void ConnectAll_EmptyCommandSet_ReturnsZeroTally()
	{
		...
		var (issuedCount, threw) = ConnectRunner.ConnectAll(Array.Empty<ConnectCommand>(), logger);
